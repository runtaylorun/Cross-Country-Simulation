    ...
    test('Given an even amount of valid teams, generate a valid schedule', () => {
        ...
        schedule.forEach(week => {
            ...
            week.racesThisWeek.forEach(race => {
                expect(uniqueTeamsRacedThisWeek.get(race.team1.teamId)).toBeFalsy()
                expect(uniqueTeamsRacedThisWeek.get(race.team2.teamId)).toBeFalsy()
        ...
        teams.forEach(team => {
            ...
            schedule.forEach(week => {
                const thisTeamsRace = week.racesThisWeek.find(race => race.team1.teamId === team.teamId || race.team2.teamId === team.teamId)

                const opponent = thisTeamsRace.team1.teamId === team.teamId ? thisTeamsRace.team2 : thisTeamsRace.team1
